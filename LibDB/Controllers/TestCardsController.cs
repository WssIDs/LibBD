...
namespace LibDB.Controllers
{
    public class TestCardsController : Controller
    {
        ...
        public ActionResult Create(string mbase)
        {
            var card = new TestCard();
            int mainbase = 1;
            if(Int32.TryParse(mbase,out mainbase))
            {
                card.BaseId = mainbase;
            }
        ...
        [HttpPost]
        public ActionResult Create(TestCard card,string mbase)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    return RedirectToAction("List", new { maingroup = mbase });
                }
                catch
                {
                    return View();
